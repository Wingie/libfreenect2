...
def project_points_to_2d(points, angle, canvas_width, canvas_height, top_view=False, 
    ...
    if top_view:
        ...
        # Skip points with zero or positive Z (behind camera with inverted Z)
        valid_idx = points_copy[:, 2] < 0
        if not np.any(valid_idx):
            return None, None, None
        
        points_copy = points_copy[valid_idx]
        
        # Calculate 2D coordinates with perspective
        # With inverted Z (negative Z is farther), we need to divide by -Z
        x_2d = focal_length * points_copy[:, 0] / -points_copy[:, 2]
        y_2d = focal_length * points_copy[:, 1] / -points_copy[:, 2]
        ...
        # Depth for coloring and point size (use absolute value for proper depth ordering)
        depth = -points_copy[:, 2]  # Convert back to positive values for easier handling
