...
try:
    ...
    while True:
        ...
        if len(points) > 0:
            ...
            # Plot points
            ax.scatter(xs, ys, zs, c=display_colors, s=2)
            
            # Set labels and title
            ax.set_xlabel('X (mm)')
            ax.set_ylabel('Y (mm)')
            ax.set_zlabel('Z (mm)')
            ax.set_title(f'Kinect Point Cloud - {len(display_points)} points')
            ...
            try:
                ...
                # Handle case where min/max might fail
                pass
        ...
        print(f"\rFrame time: {elapsed:.3f}s ({1/elapsed:.1f} FPS)", end="")
